...
var move_supply_per_unit: float = Game.SUP_PER_UNIT
var attack_supply_per_shot: float = Game.SUP_PER_SHOT
var idle_supply_rate: float = Game.SUP_IDLE_RATE
var visibility_signature: float = 1.0
var vision_radius: float = Game.VIS_UNIT
var speed: float = 0.9
var max_climb_up_steps: int = 0
var uphill_speed_mul: float = 1.0
var heading := Vector2(1.0, 0.0)
var movable: bool = true
var status_display_until: float = 0.0
var out_of_supply_started_at: float = -1.0
...
func _process(_dt: float) -> void:
	...
	self_modulate = Color(1.0, 1.0, 1.0, 0.56 if is_concealed() else 1.0)
	var visual_sig := "%0.3f|%0.3f|%0.3f|%0.3f|%s|%0.1f|%0.1f|%s" % [
		...
		hp,
		supplies,
		status_display_until > Game.elapsed or is_out_of_supply(),
	]
...
func is_out_of_supply() -> bool:
	return consumes_supplies and supplies <= 0.001

func _should_draw_status_overlay() -> bool:
	return status_display_until > Game.elapsed or is_out_of_supply()

func _draw_supply_skull(center: Vector2) -> void:
	var skull_fill := Color(0.953, 0.949, 0.929, 0.96)
	var skull_edge := Color(0.118, 0.106, 0.106, 0.92)
	_ellipse_fill(center + Vector2(0.0, -2.0), 13.6, 11.2, skull_fill, 24)
	draw_rect(Rect2(center.x - 9.6, center.y + 4.8, 19.2, 8.4), skull_fill)
	_ellipse_fill(center + Vector2(-4.8, -2.6), 2.9, 3.1, skull_edge, 16)
	_ellipse_fill(center + Vector2(4.8, -2.6), 2.9, 3.1, skull_edge, 16)
	_poly_fill([
		center + Vector2(0.0, 0.4),
		center + Vector2(-2.4, 4.8),
		center + Vector2(2.4, 4.8),
	], skull_edge)
	draw_line(center + Vector2(-5.6, 9.8), center + Vector2(5.6, 9.8), skull_edge, 2.0)
	draw_line(center + Vector2(-1.8, 7.0), center + Vector2(-1.8, 11.8), skull_edge, 1.8)
	draw_line(center + Vector2(1.8, 7.0), center + Vector2(1.8, 11.8), skull_edge, 1.8)
	_ellipse_stroke(center + Vector2(0.0, -2.0), 13.6, 11.2, skull_edge, 1.6, 24)
...
func _draw_transient_hp(sc: Vector2) -> void:
	...
	if is_out_of_supply():
		_draw_supply_skull(Vector2(sc.x, y - 10.0))
...
func _draw_movable_bars(sc: Vector2) -> void:
	...
	if is_out_of_supply():
		_draw_supply_skull(Vector2(sc.x, hy - 10.0))
